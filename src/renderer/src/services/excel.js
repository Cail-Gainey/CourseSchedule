...
export class ExcelService {
  ...
  static parseJSONData(jsonData) {
    ...
    for (let i = 0; i < Math.min(jsonData.length, 15); i++) {
      ...
      const dayCount = DAYS_OF_WEEK.filter(day => {
        ...
        return row.some(cell => {
          ...
          return cellStr.includes(day) || 
                 ...
                 (dayShort === '日' && (cellStr.includes('sunday') || cellStr === '7' || cellStr === '0'))
        })
      }).length
      
      const hasPeriodHeader = row.some(cell => 
        cell && (cell.toString().includes('节次') || 
      ...
      const hasCourseContent = row.some(cell => {
        ...
        return cellStr.length > 0 && !/^\d+$/.test(cellStr) && cellStr !== '节次' && cellStr !== '时间'
      })

      console.log(`第${i}行检测结果: 星期数=${dayCount}, 有节次标题=${hasPeriodHeader}, 有课程内容=${hasCourseContent}`)
      ...
      if (hasPeriodHeader && dayCount >= 2) {
        ...
        for (let j = 0; j < row.length; j++) {
          const cell = row[j]
          if (cell && (cell.toString().includes('节次') || cell.toString().includes('时间') || cell.toString().includes('课时'))) {
            periodColumnIndex = j
      ...
      // 次要检测：有节次标题或多个星期信息
        ...
        for (let j = 0; j < row.length; j++) {
          const cell = row[j]
          if (cell && (cell.toString().includes('节') || cell.toString().includes('时间') || cell.toString().includes('课时'))) {
            periodColumnIndex = j
            console.log(`找到节次列: 第${j}列`)
            break
          }
  ...
  static parseCourseCell(cellText, otherCells = []) {
    ...
    for (let blockIndex = 0; blockIndex < courseBlocks.length; blockIndex++) {
      ...
      if (['节次', '时间', '课时', '时段', '星期', '周'].some(keyword => courseName.includes(keyword))) {
